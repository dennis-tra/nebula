...
BEGIN
    ...
    ON CONFLICT (multi_hash) DO UPDATE
        SET updated_at=EXCLUDED.updated_at,
            agent_version_id =
                -- in case there was a new agent version discovered in this
                -- visit (and the newly discovered agent version is not
                -- null - which could happen if we couldn't dial this
                -- peer), then we update the agent version id -> same for protocols
                CASE
                    WHEN upserted_agent_version_id IS NOT NULL THEN EXCLUDED.agent_version_id
                    ELSE peers.agent_version_id END,
            protocols_set_id =
                CASE
                    WHEN upserted_protocols_set_id IS NOT NULL THEN EXCLUDED.protocols_set_id
                    ELSE peers.protocols_set_id END
