...
UPDATE peers SET
  properties = jsonb_build_object('is_exposed', is_exposed)
WHERE is_exposed IS NOT NULL;

ALTER TABLE peers DROP COLUMN is_exposed;

ALTER TABLE visits
    ADD COLUMN peer_properties JSONB;

UPDATE visits SET
    peer_properties = jsonb_build_object('is_exposed', is_exposed)
WHERE is_exposed IS NOT NULL;

ALTER TABLE visits DROP COLUMN is_exposed;
...
WITH sel AS (
    ...
    INSERT INTO peers AS p (multi_hash, agent_version_id, protocols_set_id, properties, updated_at, created_at)
        SELECT new_multi_hash, new_agent_version_id, new_protocols_set_id, new_properties, new_created_at, new_created_at
        ...
        ON CONFLICT ON CONSTRAINT uq_peers_multi_hash DO UPDATE
            SET multi_hash       = EXCLUDED.multi_hash,
                ...
                properties       = coalesce(EXCLUDED.properties, p.properties)
    ...
    UPDATE peers
        SET agent_version_id = coalesce(new_agent_version_id, agent_version_id),
            ...
            properties       = coalesce(new_properties, properties),
            updated_at       = new_created_at
        WHERE id = (SELECT id FROM sel) AND (
                (new_properties       IS NOT NULL AND coalesce(properties, '{}'::JSONB) != coalesce(new_properties, '{}'::JSONB)) OR
                (new_agent_version_id IS NOT NULL AND coalesce(agent_version_id, -1) != coalesce(new_agent_version_id, -1)) OR
                (new_protocols_set_id IS NOT NULL AND coalesce(protocols_set_id, -1) != coalesce(new_protocols_set_id, -1))
        )
        RETURNING peers.id
)
SELECT id FROM sel
...
BEGIN
    ...
    IF OLD.properties != NEW.properties THEN
        INSERT INTO peer_logs (peer_id, field, old, new, created_at)
        VALUES (NEW.id, 'properties', OLD.properties::TEXT, NEW.properties::TEXT, NOW());
    END IF;
...
BEGIN
    ...
    SELECT upsert_peer(new_peer_multi_hash, new_agent_version_id, new_protocols_set_id, new_peer_properties, new_visit_ended_at)
    INTO new_peer_id;
    ...
    INSERT INTO visits (peer_id, crawl_id, session_id, dial_duration, connect_duration, crawl_duration,
                        ...
                        agent_version_id, protocols_set_id, multi_address_ids, peer_properties)
    SELECT new_peer_id,
           new_crawl_id,
           new_session_id,
           new_dial_duration,
