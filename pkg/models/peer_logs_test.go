...
func testPeerLogsExists(t *testing.T) {
	...
	e, err := PeerLogExists(ctx, tx, o.ID, o.CreatedAt)
	if err != nil {
...
func testPeerLogsFind(t *testing.T) {
	...
	peerLogFound, err := FindPeerLog(ctx, tx, o.ID, o.CreatedAt)
	if err != nil {
		t.Error(err)
	}
...
var (
	peerLogDBTypes = map[string]string{`ID`: `integer`, `PeerID`: `integer`, `Field`: `text`, `Old`: `text`, `New`: `text`, `CreatedAt`: `timestamp with time zone`}
	_              = bytes.MinRead
)
