...
var (
	sessionDBTypes = map[string]string{`ID`: `integer`, `PeerID`: `integer`, `FirstSuccessfulDial`: `timestamp with time zone`, `LastSuccessfulDial`: `timestamp with time zone`, `NextDialAttempt`: `timestamp with time zone`, `FirstFailedDial`: `timestamp with time zone`, `MinDuration`: `interval`, `MaxDuration`: `interval`, `SuccessfulDials`: `integer`, `FinishReason`: `enum.dial_error('io_timeout','connection_refused','protocol_not_supported','peer_id_mismatch','no_route_to_host','network_unreachable','no_good_addresses','context_deadline_exceeded','no_public_ip','max_dial_attempts_exceeded','unknown','maddr_reset','stream_reset','host_is_down','negotiate_security_protocol_no_trailing_new_line')`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `Finished`: `boolean`}
	_              = bytes.MinRead
)
