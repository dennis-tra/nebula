...
type crawlR struct {
	PeerProperties PeerPropertySlice `boil:"PeerProperties" json:"PeerProperties" toml:"PeerProperties" yaml:"PeerProperties"`
}
...
// PeerProperties retrieves all the peer_property's PeerProperties with an executor.
func (o *Crawl) PeerProperties(mods ...qm.QueryMod) peerPropertyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"peer_properties\".\"crawl_id\"=?", o.ID),
	)

	query := PeerProperties(queryMods...)
	queries.SetFrom(query.Query, "\"peer_properties\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"peer_properties\".*"})
	}

	return query
}

// LoadPeerProperties allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (crawlL) LoadPeerProperties(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCrawl interface{}, mods queries.Applicator) error {
	var slice []*Crawl
	var object *Crawl
	...
	query := NewQuery(
		qm.From(`peer_properties`),
		qm.WhereIn(`peer_properties.crawl_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load peer_properties")
	}

	var resultSlice []*PeerProperty
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice peer_properties")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on peer_properties")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for peer_properties")
	}
	...
	if singular {
		object.R.PeerProperties = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &peerPropertyR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CrawlID {
				local.R.PeerProperties = append(local.R.PeerProperties, foreign)
				if foreign.R == nil {
					foreign.R = &peerPropertyR{}
				}
...
// AddPeerProperties adds the given related objects to the existing relationships
// of the crawl, optionally inserting them as new records.
// Appends related to o.R.PeerProperties.
// Sets related.R.Crawl appropriately.
func (o *Crawl) AddPeerProperties(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PeerProperty) error {
	var err error
	...
	if o.R == nil {
		o.R = &crawlR{
			PeerProperties: related,
		}
	} else {
		o.R.PeerProperties = append(o.R.PeerProperties, related...)
	}
