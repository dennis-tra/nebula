...
var (
	latencyAllColumns            = []string{"id", "peer_id", "ping_latency_s_avg", "ping_latency_s_std", "ping_latency_s_min", "ping_latency_s_max", "ping_packets_sent", "ping_packets_recv", "ping_packets_dupl", "ping_packet_loss", "updated_at", "created_at", "address"}
	latencyColumnsWithoutDefault = []string{"ping_latency_s_avg", "ping_latency_s_std", "ping_latency_s_min", "ping_latency_s_max", "ping_packets_sent", "ping_packets_recv", "ping_packets_dupl", "ping_packet_loss", "updated_at", "created_at", "address"}
	latencyColumnsWithDefault    = []string{"id", "peer_id"}
	latencyPrimaryKeyColumns     = []string{"id"}
	latencyGeneratedColumns      = []string{}
)
...
func (latencyL) LoadPeer(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLatency interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeLatency.(*Latency)
		if !ok {
			object = new(Latency)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeLatency)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeLatency))
			}
		}
	} else {
		s, ok := maybeLatency.(*[]*Latency)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeLatency)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeLatency))
			}
		}
	}
...
func Latencies(mods ...qm.QueryMod) latencyQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"latencies\".*"})
	}

	return latencyQuery{q}
}
