...
var (
	agentVersionAllColumns            = []string{"id", "updated_at", "created_at", "agent_version"}
	agentVersionColumnsWithoutDefault = []string{"updated_at", "created_at", "agent_version"}
...
func (agentVersionL) LoadCrawlProperties(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAgentVersion interface{}, mods queries.Applicator) error {
	...
	if singular {
		object = maybeAgentVersion.(*AgentVersion)
	} else {
		slice = *maybeAgentVersion.(*[]*AgentVersion)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (agentVersionL) LoadVisits(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAgentVersion interface{}, mods queries.Applicator) error {
	var slice []*AgentVersion
	var object *AgentVersion
	...
	query := NewQuery(
		qm.From(`visits`),
		qm.WhereIn(`visits.agent_version_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func AgentVersions(mods ...qm.QueryMod) agentVersionQuery {
	...
	return agentVersionQuery{NewQuery(mods...)}
}
