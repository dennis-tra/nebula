...
var (
	peerDBTypes = map[string]string{`MultiHash`: `character varying`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `AgentVersion`: `character varying`, `Protocol`: `ARRAYcharacter varying`, `ID`: `integer`}
	_           = bytes.MinRead
)
