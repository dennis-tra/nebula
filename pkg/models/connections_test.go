...
var (
	connectionDBTypes = map[string]string{`ID`: `integer`, `PeerID`: `character varying`, `MultiAddress`: `character varying`, `AgentVersion`: `character varying`, `DialAttempt`: `timestamp with time zone`, `Latency`: `interval`, `IsSucceed`: `boolean`}
	_                 = bytes.MinRead
)
