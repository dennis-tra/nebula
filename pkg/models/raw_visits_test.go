...
var (
	rawVisitDBTypes = map[string]string{`ID`: `integer`, `CrawlID`: `integer`, `ConnectLatency`: `interval`, `ConnectStartedAt`: `timestamp with time zone`, `PeerMultiHash`: `character varying`, `AgentVersion`: `character varying`, `Protocols`: `ARRAYcharacter varying`, `MultiAddresses`: `ARRAYcharacter varying`, `Error`: `text`, `CreatedAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)
