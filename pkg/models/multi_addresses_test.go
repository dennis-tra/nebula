...
func testMultiAddressToManyMultiAddressesXIPAddresses(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadMultiAddressesXIPAddresses(ctx, tx, false, (*[]*MultiAddress)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testMultiAddressToManyPeers(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"peers_x_multi_addresses\" (\"multi_address_id\", \"peer_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"peers_x_multi_addresses\" (\"multi_address_id\", \"peer_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testMultiAddressToManyVisits(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"visits_x_multi_addresses\" (\"multi_address_id\", \"visit_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"visits_x_multi_addresses\" (\"multi_address_id\", \"visit_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := MultiAddressSlice{&a}
	if err = a.L.LoadVisits(ctx, tx, false, (*[]*MultiAddress)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testMultiAddressToManyAddOpMultiAddressesXIPAddresses(t *testing.T) {
	var err error
...
func testMultiAddressToManyAddOpVisits(t *testing.T) {
	var err error
...
func testMultiAddressToManySetOpVisits(t *testing.T) {
	var err error
...
func testMultiAddressToManyRemoveOpVisits(t *testing.T) {
	var err error
...
var (
	multiAddressDBTypes = map[string]string{`ID`: `integer`, `Maddr`: `character varying`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_                   = bytes.MinRead
)
