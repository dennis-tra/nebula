...
var (
	protocolDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `Protocol`: `text`}
	_               = bytes.MinRead
)
...
func testProtocolsSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(protocolAllColumns, protocolPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, protocolGeneratedColumns)
	}
